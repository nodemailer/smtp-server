...
// Control characters have no legitimate use in usernames or command parameters.
// They enable log and header injection in applications that embed such a value
// unescaped, and they can split an SMTP response, so reject them everywhere.
// C1 controls (\x80-\x9F) are included because U+0085 is a line terminator for
// many parsers. Shared with lib/smtp-connection.js - keep a single definition
...
const SASL = (module.exports = {
    ...
    'SASL_CRAM-MD5'(args, callback) {
        ...
        // the challenge must be unpredictable, so use a CSPRNG instead of
        // Math.random() whose internal state can be recovered from outputs
    ...
    PLAIN_token(canAbort, token, callback) {
        ...
        // RFC 4616: the token is authzid NUL authcid NUL passwd. The authentication
        // identity is the authcid, but many real-world clients place the username in
        // the authzid field and leave the authcid empty, so fall back to the authzid
        // in that case. Both raw fields are passed to onAuth so applications that
        // need strict RFC 4616 semantics can implement them
        let authzid = data[0] || '';
        let authcid = data[1] || '';
        ...
        this._server.onAuth(
            ...
            (err, response) => {
                ...
                response = response || {}; // onAuth may signal a failure with a bare callback()
        ...
        this._server.onAuth(
            {
                ...
                validatePassword(password) {
                    ...
                    let expected = Buffer.from(hmac.update(challenge).digest('hex').toLowerCase());
                    let received = Buffer.from(challengeResponse);
    ...
    SASL_XCLIENT(args, callback) {
        ...
        this._server.onAuth(
            ...
            (err, response) => {
                ...
                if (!response) {
                    // A bare callback() means the application did not establish an
                    // identity for the LOGIN value but does not object to it either.
                    // This is what the built-in onAuth handler does, so let the
                    // XCLIENT command through with an unauthenticated session
                    this._server.logger.info(
                        {
                            tnx: 'authskip',
                            cid: this.id,
                        ...
                        'Authentication skipped for %s using %s',
                        username,
                        'XCLIENT'
                    );
                    return callback();
...
// Not a mechanism, exposed so that the rest of the library shares one definition.
// Mechanism lookups use the "SASL_" prefix, so this can not collide with one
module.exports.CONTROL_CHARS = CONTROL_CHARS;
