...
class SMTPConnection extends EventEmitter {
    ...
    handler_MAIL(command, callback) {
        ...
        // Process BODY parameter if present (RFC 6152 - 8BITMIME)
        if (parsed.args.BODY) {
            const bodyType = parsed.args.BODY.toUpperCase();
            if (bodyType !== '7BIT' && bodyType !== '8BITMIME' && bodyType !== 'BINARYMIME') {
                this.send(501, 'Error: BODY parameter must be 7BIT, 8BITMIME, or BINARYMIME');
                return callback();
            }
            this.session.envelope.bodyType = bodyType;
        }

        // Process SMTPUTF8 parameter if present (RFC 6531)
        if (parsed.args.SMTPUTF8 !== undefined) {
            // SMTPUTF8 parameter has no value (it's a flag)
            if (parsed.args.SMTPUTF8 !== true) {
                this.send(501, 'Error: SMTPUTF8 does not take a value');
                return callback();
            }
            this.session.envelope.smtpUtf8 = true;
        }

        // Process REQUIRETLS parameter if present (RFC 8689)
        if (parsed.args.REQUIRETLS !== undefined) {
            // REQUIRETLS can only be used over a TLS connection
            if (!this.secure) {
                this.send(530, 'Error: REQUIRETLS requires TLS connection');
                return callback();
            }
            // REQUIRETLS parameter has no value (it's a flag)
            if (parsed.args.REQUIRETLS !== true) {
                this.send(501, 'Error: REQUIRETLS does not take a value');
                return callback();
            }
