...
class SMTPConnection extends EventEmitter {
    ...
    _parseAddressCommand(name, command) {
        ...
        if (address && /[\x00-\x1F\x7F-\x9F\u200B\u2060\uFEFF]/.test(address)) {
            // Control characters and invisible Unicode formatting characters
            // (zero-width space, word joiner, zero-width no-break space) are never
            // legitimate in addresses; they enable log/header injection and
            // visually spoofed addresses, so reject them even in lenient mode.
            // Zero-width joiners (U+200C, U+200D) are not rejected as these may
            // appear in internationalized domain names of some scripts
            invalid = true;
        } else if (address) {
