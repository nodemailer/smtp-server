...
class SMTPConnection extends EventEmitter {
    ...
    connectionReady(next) {
        ...
        if (this._server.options.resolver && typeof this._server.options.resolver.reverse === 'function') {
            try {
                this._server.options.resolver.reverse(this.remoteAddress.toString(), (...args) => {
                    clearTimeout(reverseTimer);
                    if (greetingSent) {
                        return;
                    }
    ...
    _onCommand(command, callback) {
        ...
        // Check if server is closing and reject commands if so
        if (this._server._closeTimeout) {
            return callback(new Error('Server is shutting down'));
        }
