            ...
            if (address.length !== 2 || !address[0] || !address[1]) {
                ...
                if (localPart.length + 1 + domain.length > 253) {
                    ...
                    if (isIPLiteral) {
                        ...
                        // Check if IP literals are allowed via server options
                        const allowIPLiteral = this._server.options.allowIPLiteralAddresses;

                        if (!allowIPLiteral) {
                            // IP literals are disabled by default for security
                            invalid = true;
                        } else {
